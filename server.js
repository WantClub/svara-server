...
app.post('/api/register', authLimiter, (req, res) => {
  ...
  // Раньше здесь была проверка "не регистрировался ли уже кто-то с этого
  // IP" — она должна была отсекать повторный фарм бонуса, но, судя по
  // всему, на самом хостинге (за прокси Render) IP-адрес не всегда
  // определяется корректно, из-за чего проверка ошибочно решала, что
  // "с этого адреса уже кто-то был", и НАСТОЯЩИЕ новые игроки оставались
  // без бонуса. Обломанная защита от фрода хуже, чем полное её отсутствие —
  // убираю проверку, бонус теперь получает каждый новый аккаунт.
